...
class TitleVC: UIViewController, UITextFieldDelegate {
    ...
    @objc func handleNext() {
        if let text = inputTextField.text, text.count <= 30, text.isEmpty == false {
            let descriptionVC = DescriptionVC()
            descriptionVC.groupTitle = self.inputTextField.text
            self.show(descriptionVC, sender: self)
        }
    }
